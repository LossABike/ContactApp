...
exports.findAll = async (req, res, next) => {
    ...
    if (name) {
        condition.name = { $regex: newRegExp(name), $options: "i" };
    }
...
exports.findOne = async (req, res, next) => {
    ...
    if (error) {
        return next(new BadRequestError(500, `Error retireving contact with id=${req.params.id}`));
    }
...
exports.update = async (req, res, next) => {
    ...
    if (error) {
        return next(new BadRequestError(500, `Error updateting contact with id=${req.params.id}`));
    }
    if (!document) {
        return next(new BadRequestError(404, `Contact not found`));
...
exports.findAllFavorite = async (req, res, next) => {
    ...
    if (error) {
        return next(new BadRequestError(500, 'An error occured while retrieving favorite contacts'));
...
exports.deleteAll = async (req, res, next) => {
    ...
    if (error) {
        return next(new BadRequestError(500, 'An error occured while removing all contacts'));

    }
    return res.send({
        message: `${data.deletedCount} contacts were successfully deleted`,
    });
